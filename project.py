...
def queryData():
    allCategory = session.query(MedCategory)\
            .order_by(asc(MedCategory.category))
...
def showUserJSON():
    ...
    return jsonify(allUsers=[i.serialize for i in users],
                   message="All users list created successfully")
...
def showAllMedsJSON():
    a, b = queryData()
    return jsonify(AllMEDS=[i.serialize for i in b],
                   message="All medication list created successfully")
...
def allCategoryJSON():
    a, b = queryData()
    return jsonify(Category=[i.serialize for i in a],
...
def oneMedicationJSON(category, med):
    try:
        ...
        return jsonify(message="Please ensure that your are searching"
                       ...
                       Message=" .... Sorry we can't find your medication in"
...
def medicationlist():
    medicationclass = session.query(MedCategory)\
                .order_by(asc(MedCategory.category))
...
def cancelDeletion():
    candel = session.query(MedCategory)\
...
def medsCatList(medcat):
    medicationlist = session.query(MedCategory)\
                .order_by(asc(MedCategory.category))
    categoryM = session.query(MedCategory).filter_by(category=medcat).first()
    medsdisplay = session.query(MedList)\
        .filter_by(medcategory_id=categoryM.id)\
...
def medList(medcat, med):
    ...
    if 'username' in login_session:
        return render_template('catMeds.html',
                               categoryM=categoryM,
                               medsdisplay=medsdisplay, caMed=caMed)
    else:
        return render_template('catMedsPublic.html',
                               categoryM=categoryM,
                               medsdisplay=medsdisplay, caMed=caMed)
    ...
    if request.method == 'POST':
        if request.form['category']:
            ...
            return redirect(url_for
                            ('medsCatList', medcat=request.form['category']))
        else:
...
def createMedication(medcat):
    ...
    if request.method == 'POST':
        if request.form['name']:
            newMed = MedList(name=request.form['name'],
                             description=request.form['description'],
                             adverseEffect=request.form['adverseEffect'],
                             pregnancyCategory=request
                             .form['pregnancyCategory'],
                             medcategory_id=caMed.id, user_id=caMed.user_id)
            add_entry(newMed)
            flash("%s created in %s category!" % (request.form['name'],
                                                  medcat))
...
@app.route('/medication/<string:medcat>/<string:med>/Edit',
           methods=['GET', 'POST'])
def editMedication(medcat, med):
    ...
    if medToEdit.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized" \
               "to edit any medication this medication category. Please" \
               "create your own category in order to edit any medication" \
               "in the category.');}</script><body onload='myFunction()'>"
    if request.method == 'POST':
        if request.form['name']:
            ...
            add_entry(medToEdit)
            flash("%s updated in %s category!" % (request.form['name'],
                                                  medcat))
            return redirect(url_for('medList', medcat=editcat.category,
                                    med=medToEdit.name))
        else:
...
@app.route('/medication/<string:medcat>/<string:med>/Delete/',
           methods=['GET', 'POST'])
def deleteMedication(medcat, med):
    ...
    if medTodel.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized" \
               "to delete any medication this medication category. Please" \
               "create your own category in order to delete any medication" \
               "in the category.');}</script><body onload='myFunction()'>"
    if request.method == 'POST':
        delete_entry(medTodel)
        flash("%s deleted from %s category!" % (med, medcat))
        medTodel2 = session.query(MedList)\
            .filter_by(medcategory_id=deletecat.id)\
            .order_by(asc(MedList.name)).first()
        return redirect(url_for('medList', medcat=deletecat.category,
                                med=medTodel2.name))
    else:
